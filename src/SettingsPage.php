...
class SettingsPage {
	...
	public function add_admin_menu(): void { 
		add_options_page( __( 'jQuery Version Control', 'version-control-for-jquery' ), __( 'jQuery Version Control', 'version-control-for-jquery' ), 'manage_options', 'version_control_for_jquery', [$this, 'render_page'] );
	}
	...
	public function select_core_version(): void { 
		...
		$version = $options['vcfj_core_version'] ?? VCFJ::$latest_core;
	...
	public function select_migrate_version(): void {
		...
		$options = get_option( 'vcfj_settings' );
		$version = $options['vcfj_migrate_version'] ?? VCFJ::$latest_migrate;
	...
	public function migrate_disable(): void {
		$options = get_option( 'vcfj_settings' );
	...
	private function output_option(string $name, $current, $version, $label): void {
		...
		echo sprintf( '<option value="%1$s" name="vcfj_settings[%2$s]" %3$s>jQuery %4$s %5$s</option>', $version, $setting_name, selected( $current, $version ), $option_label_prefix, $label );
	}

	public function section_callback(): void {
		echo sprintf( '<p>%s</p>', __( 'Use the dropdown selectors below to select your desired version of jQuery. Please note that the plugin defaults to the latest stable version.', 'version-control-for-jquery' ) ); 
	}
	...
	public function set_core_version(): void {
	...
	public function set_migrate_version(): void {
		// Check that the user is not viewing the administration panel
		if( !is_admin() ) {
